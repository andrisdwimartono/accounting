    ...
    $.ajax({
        ...
        success: function(data){
            for(var i = 0; i < Object.keys(data.data.{{$page_data["page_data_urlname"]}}).length; i++){
                if(Object.keys(data.data.{{$page_data["page_data_urlname"]}})[i] == "unit_pelaksana" || Object.keys(data.data.{{$page_data["page_data_urlname"]}})[i] == "programkerja"){
    ...
    $("#caktable4 > tbody > tr").each(function(index, tr){
        ...
        $(tr).find("td").each(function(index, td){
            if(index == 0){
                ...
                desc_detail4 = $(td).find("input").val();
            }
        });
        if(detailbiayaproker_name != '')
            ctct4_detailkegiatan.push({"no_seq": index, "detailbiayaproker_name": detailbiayaproker_name, "deskripsibiaya": deskripsibiaya, "volume": volume, "satuan": satuan, "satuan_label": satuan_label, "standarbiaya": standarbiaya, "id": id, "komentarrevisi": komentarrevisi, "status": status4, 'desc_detail': desc_detail4});
    });
        ...
        $("#caktable4").find('tbody')
            .append(
                @if($page_data["page_method_name"] == "View")
                    ...
                    +"<td class=\"p-0\"><input type=\"text\" name=\"desc_detail4_"+rowlen+"\" class=\"form-control form-control-sm\" id=\"desc_detail4_"+rowlen+"\"></td>"
                    +"<td class=\"column-hidden\"></td>"
    ...
    $.validator.setDefaults({
        submitHandler: function (form, event) {
            ...
            $("#caktable4 > tbody > tr").each(function(index, tr){
                ...
                if(detailbiayaproker_name != '')
                    ctct4_detailkegiatan.push({"no_seq": index, "detailbiayaproker_name": detailbiayaproker_name, "deskripsibiaya": deskripsibiaya, "volume": volume, "satuan": satuan, "satuan_label": satuan_label, "standarbiaya": standarbiaya, "id": id, "desc_detail": desc_detail4});
            });
