    ...
    <body>
        <table border="1" width="490px" class="table">
            ...
            <tbody>
                ...
                <tr>
                    @if($transaction[6]==0)
                    <td colspan="4" style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">{{ $transaction[6]==0?$transaction[2]:"" }}</td>
                    @elseif($transaction[6]==1)
                    <td></td>
                    <td colspan="3" style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">{{ $transaction[6]==1?$transaction[2]:"" }}</td>
                    @elseif($transaction[6]==2)
                    <td></td>
                    <td></td>
                    <td colspan="2" style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">{{ $transaction[6]==2?$transaction[2]:"" }}</td>
                    @elseif($transaction[6]==3)
                    <td></td>
                    <td></td>
                    <td></td>
                    <td style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">{{ $transaction[6]==3?$transaction[2]:"" }}</td>
                    @endif
                    <td></td>
                    <td style="text-align: right;">{{ is_float($transaction[4])?($transaction[7]!='on'?(((double)$transaction[4])>0||$transaction[4]!="0"?number_format($transaction[4], 0, ",", "."):number_format(0, 0, ",", ".")):""):0 }}</td>
                    <td style="text-align: right;">{{ is_float($transaction[3])?($transaction[7]!='on'?(((double)$transaction[3])>0||$transaction[3]!="0"?number_format($transaction[3], 0, ",", "."):number_format(0, 0, ",", ".")):""):0 }}</td>
                </tr>
